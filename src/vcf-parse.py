    ...
    for line in args.vcf:
        ...
        if not line.startswith("#"):
            ...
            if len(vcf[4]) in bc_lens and len(Counter(vcf[3])) == 1:
                barcodes.append((vcf[0], vcf[1], vcf[4], rev_comp(vcf[4])))
                bc_lens.remove(len(vcf[4]))

    # graceful exit for no barcode
    n_barcodes = len(barcodes)
    if n_barcodes == 0:
        print(f'{ref}\t1\tNA\tNA\tNA\t{n_vars}\t0\t{expected_bcs}',
              file=sys.stdout)
    # taking advantage of new f-strings
    for idx, tmp in enumerate(barcodes):
        ref, pos, bc, bc_revcomp = tmp
        print(
            f'{ref}\t{idx+1}\t{bc}\t{bc_revcomp}\t{pos}\t{n_vars}\t{n_barcodes}\t{expected_bcs}',
            file=sys.stdout)
